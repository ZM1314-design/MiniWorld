# miniworld: 从零开始训练个人亿级参数大模型（最小数据量【hugging face】数据闭环）
# 如果算力充足，可从零开始，随机初始化权重，构造个人10亿级参数的私有大模型。或者继续预训练百亿级参数大模型，精度会更高，下一步的微调效果也会更理想
...
## 继续预训练（CFT / DAPT）：Qwen-0.5B × 农业语料（单卡 4090）

如果你的目标是“对基座大模型做农业领域继续预训练（非 SFT）”，请直接看这个子项目：

- `llm-continue_pretrain-CFT/`（包含数据准备脚本、Axolotl 训练配置与完整说明）
